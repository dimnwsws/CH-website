...
burger.addEventListener('click', () => {
    if (navMenu.classList.contains('is-open')) {
        ...
        BagIcon.setAttribute('src', 'bag.svg');
        headerIcon.setAttribute('src', 'logo.svg');
        headerChanged.style.backgroundColor = "white";

    } else {
        navMenu.classList.add('is-open');
        headerChanged.style.backgroundColor = "rgba(168, 123, 98, 1)";
        burger.setAttribute('src', 'burgerwhite.svg');
        BagIcon.setAttribute('src', 'bagwhite.svg');
        headerIcon.setAttribute('src', 'logowhite.svg');
    }
});
...
expandSvgs.forEach((expandSvg, index) => {
    expandSvg.addEventListener('click', () => {
        if (expandTexts[index].style.display === 'none') {
            ...
            expandSvg.setAttribute('src', 'open.svg');
        } else {
            expandTexts[index].style.display = 'none';
            expandSvg.setAttribute('src', 'close.svg');
        }
    });
});
